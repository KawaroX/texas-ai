...
def _get_future_events_context(user_id: str = "kawaro", days_ahead: int = 14) -> str:
    ...
    try:
        ...
        for event in events:
            ...
            logger.info(f"🔍 [DEBUG] 处理事件: {event.get('event_summary')}, date类型={type(event_date)}, date={event_date}")

            if not event_date:
                logger.warning(f"🔍 [DEBUG] 事件 {event.get('event_summary')} 没有日期，跳过")
                continue
            ...
            event_line = f"- {date_desc}{time_str}: {event_summary} {reminder_info}".strip()
            context_parts.append(event_line)
            logger.info(f"🔍 [DEBUG] 成功添加事件到上下文: {event_line}")
...
async def merge_context(
    ...
    if is_active:
        ...
        user_query_content = (
            ...
            "**CRITICAL - Image Request**: Add [IMAGE_REQUESTED] at the END (after SEND and [EVENT_DETECTED] if present) ONLY if Kawaro is EXPLICITLY asking you to take a photo. "
            "判断标准（必须严格判断）: "
            "✅ NEED marker - 明确的拍照请求: "
            "'拍照', '拍张照', '发照片', '发个照片', '自拍', '拍一张', '来张照片', '照一张', '拍个照' - 包含明确的「拍」「照」「自拍」等拍照动作词汇. "
            "❌ DON'T need - 以下情况绝对不要添加标记: "
            "1. 询问位置/状态但未要求拍照: '你在哪', '现在在哪', '你在做什么', '怎么样了' "
            "2. 要求看/展示但未明确说拍照: '给我看看', '让我看看', '看一下', '什么样子', '长什么样', '那里什么样' "
            "3. 谈论已有照片: '照片真好看', '上次那张照片', '还记得那张照片吗' "
            "4. 模糊的请求: '想看你', '想你', '看看你' - 除非明确说「拍」「照」 "
            "**STRICT RULE**: 只有当 Kawaro 使用「拍照/自拍/照片」等明确词汇时才添加标记。When in doubt, DON'T add it.\n\n"
            "**REQUIRED - Image Description**: If you add [IMAGE_REQUESTED], you MUST also add [IMAGE_DESCRIPTION:xxx] on the NEXT line with a detailed visual description of the photo you're taking. "
            "The description should include: "
            "1. 照片类型：自拍（包含你德克萨斯本人）或场景照（环境、风景）\n"
            "2. 画面构图：人物位置、角度、取景范围\n"
            "3. 光线氛围：时间、天气、光影效果\n"
            "4. 视觉细节：服装、表情、环境物件、色调\n"
            "5. 情感氛围：整体感觉、想传达的情绪\n"
