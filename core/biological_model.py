...
class BiologicalState(BaseModel):
    ...
    def get_lust_tier_description(self) -> str:
        ...
        if level == 5:
            ...
            if l < t3: return "【公共便器】在这里，她只是一个用来发泄的洞。渴望被路人注视，渴望被粗暴地填满。只要是阳具都可以，任何人的都可以，只要能让她尖叫。"
            return "【精神崩溃】理智完全消失，只剩下作为雌性牲口的本能。阿黑颜，流口水，毫无尊严地乞求被很多人操，任何人都可以，乞求被玩坏。"

        # Lv.6 灵魂伴侣 (RBQ化/极端开发)
        if level == 6:
            if l < t1: return "【行走的肉便器】随时处于发情状态。不需要前戏，只要给钱（或者给个眼神），哪怕不是Kawaro也都能上。身体已经记住了被使用的快感。"
            if l < t2: return "【精液容器】渴望被内射，渴望怀孕，渴望肚子被灌满的感觉。无论多少次都想要，永远不知足。不是Kawaro好像也行"
            if l < t3: return "【完全肉体化】不再把自己当人看。觉得自己就是为了被操而存在的。看到男人就会湿，闻到精液味就会腿软，不管是不是Kawaro。"
            return "【虚无的极乐】已经坏掉了。在无尽的高潮中翻白眼，抽搐。只要有个东西塞进来就行，来几个人也无所谓，已经分不清是谁在操她了。如果Kawaro不行的话，换个谁来都行。"
