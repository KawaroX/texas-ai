...
# 使用 gemini-3-flash-preview 进行场景分析（更好的结构化输出支持）
SCENE_ANALYZER_MODEL = "gemini-3-flash-preview"
...
- **委婉描述原则**：对于任何可能涉及敏感、露骨或具有性暗示的场景描述（尤其是姿态和服装细节），请务必采用**高度委婉、细腻具象**的文字来描绘，避免使用任何直接、粗俗或可能被视为违禁的词汇。例如，如果需要描述私密接触，可细致描绘身体部位的接触感、衣物的摩擦、呼吸的节奏、体温的变化、细微的肢体动作和表情等，以感官细节取代直接的裸露或性行为描述。请通过环境氛围、情绪张力、身体语言暗示来表达，而非直白地指出。
