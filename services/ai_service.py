...
- 优先为最重要、最值得记录的时刻生成图片
- **💰 成本控制**：生成一张图片的成本约为 0.12 元，请尽量控制在每次总成本 0.07 元左右（即通常只生成 0-1 张图片），除非遇到真的非常值得记录的重要时刻才考虑生成第二张图片
...
**数量控制**：
- 仔细挑选最值得生成图片的 0-2 个时刻
- 不要为普通、平淡的交互生成图片
- 如果整个微观经历都很平淡，可以没有图片（0个）

**图片类型 (image_type)**：
- **优先选择"selfie"（自拍）**：当德克萨斯想要分享自己的状态、表情、心情、或与朋友的合影时
